<template>
  <view>
      ...
      <view style="background: #ffffff; border-radius: 10rpx;">
        ...
        <view style="padding: 10rpx;">
          <view style="display: flex; flex-direction: row;" v-for="(item,i) in 4" :key="i">
            <view v-for="(item,j) in 5" style="" class="week-bt"
              :style="(5*i + j + 1)==nowWeek?'background: #39b54a; color: white;':''" :key="j"
    ...
    <view>
      ...
      <view style="width: 100%; display: flex;" :style="{ 'padding-top': statusBarHeight + 57 + 'px' }">
        <view>
...
<script setup>
  ...
  const props = defineProps({
    ...
    courseList: {
      type: Array,
      // 重构 v-if="(nowWeek>=item.week_begin&&nowWeek<=item.week_end) || showNotWeek" 
      default: (courseList) => courseList.filter((item) => {
        return (week >= item.week_begin && week <= item.week_end) || showNotWeek
      })
    }
  });

  const width = ref(uni.getSystemInfoSync().windowWidth);
